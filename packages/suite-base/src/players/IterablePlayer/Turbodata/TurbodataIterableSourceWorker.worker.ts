...
import { TurbodataIterableSource } from "./TurbodataIterableSource";

export function initialize(
  args: IterableSourceInitializeArgs,
): WorkerSerializedIterableSourceWorker {
  if (args.file) {
    const source = new TurbodataIterableSource({ type: "file", file: args.file });
    ...
    const source = new MultiIterableSource(
      ...
      TurbodataIterableSource,
    );
    const wrapped = new WorkerSerializedIterableSourceWorker(source);
    return Comlink.proxy(wrapped);
  } else if (args.url) {
    const source = new TurbodataIterableSource({ type: "url", url: args.url });
    const wrapped = new WorkerSerializedIterableSourceWorker(source);
    return Comlink.proxy(wrapped);
  } else if (args.urls) {
    const source = new MultiIterableSource(
      { type: "urls", urls: args.urls },
      TurbodataIterableSource,
    );
    const wrapped = new WorkerSerializedIterableSourceWorker(source);
    return Comlink.proxy(wrapped);
